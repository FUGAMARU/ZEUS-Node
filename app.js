...
const getTwitterTrends = () => {
	client.get("trends/place", {id: 23424856}, (err, res) => {
		if(!err){
			...
			trends = [{name: sortedTrends[0].name, url: sortedTrends[0].url}, {name: sortedTrends[1].name, url: sortedTrends[1].url}, {name: sortedTrends[2].name, url: sortedTrends[2].url}]
...
io.on("connection", (socket) => {
	...
	socket.on("requestTrends", () => {
		socket.emit("receiveTrends", trends)
	})
})